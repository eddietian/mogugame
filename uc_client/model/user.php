...
class usermodel {
	...
	function add_user($username, $password,$email,$promote_id,$promote_account,$game_id,$game_name,$version, $uid = 0, $questionid = '', $answer = '', $regip = '',$platform='') {
		$regip = empty($regip) ? $this->base->onlineip : $regip;
		$salt = substr(uniqid(rand()), -6);
		$password = md5(md5($password).$salt);
		$sqladd = $uid ? "uid='".intval($uid)."'," : '';
		$sqladd .= $questionid > 0 ? " secques='".$this->quescrypt($questionid, $answer)."'," : " secques='',";
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd username='$username', password='$password',email='$email',promote_id='$promote_id',promote_account='$promote_account',game_id='$game_id',game_name='$game_name',version='$version', regip='$regip', regdate='".$this->base->time."', salt='$salt',platform='$platform'");
