...
*/
include './Application/Ucenter/Conf/config.php';
if(!defined('UC_API')) {
	exit('Access denied');
}
...
function uc_user_register($username, $password,$email,$promote_id=0,$promote_account='自然注册',$game_id=0,$game_name="", $version,$questionid = '', $answer = '', $regip = '',$platform='') {
	return call_user_func(UC_API_FUNC, 'user', 'register', array('username'=>$username, 'password'=>$password, 'email'=>$email, 'promote_id'=>$promote_id,'promote_account'=>$promote_account,'game_id'=>$game_id,'game_name'=>$game_name,'version'=>$version,'questionid'=>$questionid, 'answer'=>$answer, 'regip' => $regip,'platform'=>$platform));
}
...
function uc_get_user_by_username($username){
	$return = call_user_func(UC_API_FUNC, 'user', 'get_user_by_username',$username);
	return $return;
}
function uc_get_user_by_uid($uid){
	$return = call_user_func(UC_API_FUNC, 'user', 'get_user_by_uid',$uid);
	return $return;
}
...
//uc新增充值记录
function uc_add_spend($uid,$uaccount,$unick,$gid,$gappid,$gname,$sid,$sname,$pid,$paccount,$orderNo,$payOrderNo,$payAmount,$payTime,$extend,$payWay,$spendIp,$sdkVer,$version,$platform,$pay_notify_url,$game_key){
	return call_user_func(UC_API_FUNC, 'spend', 'add', array('user_id'=>$uid, 'user_account'=>$uaccount,'user_nickname'=>$unick,'game_id'=>$gid,'game_appid'=>$gappid,'game_name'=>$gname,'server_id'=>$sid,'server_name'=>$sname,'promote_id'=>$pid,'promote_account'=>$paccount,'order_number'=>$orderNo,'pay_order_number'=>$payOrderNo,'pay_amount'=>$payAmount,'pay_time'=>$payTime,'extend'=>$extend,'pay_way'=>$payWay,'spend_ip'=>$spendIp,'sdk_version'=>$sdkVer,'version'=>$version,'platform'=>$platform,'pay_notify_url'=>$pay_notify_url,'game_key'=>$game_key));
}
//uc更改充值记录状态
function uc_edit_spend($payOrderNo,$payStatus,$payGameStatus){
	return call_user_func(UC_API_FUNC,'spend','edit',array('pay_order_number'=>$payOrderNo,'pay_status'=>$payStatus,'pay_game_status'=>$payGameStatus));
}
function uc_edit_deposit($payOrderNo,$payStatus){
	return call_user_func(UC_API_FUNC,'deposit','edit',array('pay_order_number'=>$payOrderNo,'pay_status'=>$payStatus));
}
function uc_find_spend($payOrderNo){
	return call_user_func(UC_API_FUNC,'spend','find',array('pay_order_number'=>$payOrderNo));
}
function uc_find_deposit($payOrderNo){
	return call_user_func(UC_API_FUNC,'deposit','find',array('pay_order_number'=>$payOrderNo));
}
function uc_change_spend($payOrderNo,$status){
	return call_user_func(UC_API_FUNC,'spend','change',array('pay_order_number'=>$payOrderNo,'status'=>$status));
}
function uc_select_spend($page=1,$ppp=10,$param){
	return call_user_func(UC_API_FUNC,'spend','select',array('page'=>$page,'ppp'=>$ppp,'param'=>$param));
}
function uc_select_deposit($page=1,$ppp=10,$param,$for_uc=''){
	return call_user_func(UC_API_FUNC,'deposit','select',array('page'=>$page,'ppp'=>$ppp,'param'=>$param,'for_uc'=>$for_uc));
}
//uc新增平台币充值记录
function uc_add_deposit($uid,$uaccount,$unick,$gid,$gappid,$gname,$sid,$sname,$pid,$paccount,$orderNo,$payOrderNo,$payAmount,$payTime,$extend,$payWay,$spendIp,$sdkVer,$version,$platform,$pay_notify_url,$game_key){
	return call_user_func(UC_API_FUNC, 'deposit', 'add', array('user_id'=>$uid, 'user_account'=>$uaccount,'user_nickname'=>$unick,'game_id'=>$gid,'game_appid'=>$gappid,'game_name'=>$gname,'server_id'=>$sid,'server_name'=>$sname,'promote_id'=>$pid,'promote_account'=>$paccount,'order_number'=>$orderNo,'pay_order_number'=>$payOrderNo,'pay_amount'=>$payAmount,'create_time'=>$payTime,'extend'=>$extend,'pay_way'=>$payWay,'deposit_ip'=>$spendIp,'sdk_version'=>$sdkVer,'version'=>$version,'platform'=>$platform,'pay_notify_url'=>$pay_notify_url,'game_key'=>$game_key));
}
?>