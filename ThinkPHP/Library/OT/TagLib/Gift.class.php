...
class Gift extends TagLib{
     ...
     */
        'index'    => array('attr' => 'name,recommend_status,page,sort,limit,field,isrecord', 'close' => 1), 
        'record'    => array('attr' => 'name,recommend_status,limit', 'close' => 1), 
    );
    
    public function _record($tag, $content){
        $name   = $tag['name'];
        $cate   = empty($tag['recommend_status'])?'':$tag['recommend_status'];
        $limit  = empty($tag['limit'])   ? '10' : $tag['limit'];
        
        $parse  = '<?php ';
        $parse .= '$__LIST__ = D(\'Giftbag\')->record(';
        $parse .= '\''.$cate.'\','.$limit;
        $parse .= ');';
        $parse .= ' ?>';
    ...
    public function _index($tag, $content){
        ...
        $sort   = empty($tag['sort'])   ? 'tab_giftbag.id desc' : $tag['sort'];
        $limit  = empty($tag['limit'])   ? '10' : $tag['limit'];
        $field  = empty($tag['field']) ? 'true' : $tag['field'];
        $isrecord  = empty($tag['isrecord']) ? 'false' : $tag['isrecord'];
        
        $parse  = '<?php ';
        $parse .= '$__LIST__ = D(\'Giftbag\')->index(';
        $parse .= '\''.$cate.'\','.$page.', \''.$sort.'\','.$limit.',';
        $parse .= $field .','.$isrecord.');';
        $parse .= ' ?>';
        $parse .= '<volist name="__LIST__" id="'. $name .'">';
        $parse .= $content;
        $parse .= '</volist>';
