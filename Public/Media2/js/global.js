var SURL = '/media2.php?s=/',
	UURL = '/media2.php?s=/Member/';
// xigu  lwx
(function() {
    ...
    (function() {
        ...
        XGUC.reg = function(pop_id) {
			...
			$('.checkyzm').on('click',function() {
				var e = (new Date).getTime();
				$(this).attr('src', UURL+'verify/vid/666/t/' + e);
			});
		...
		XGUC.phone_reg = function(form_id) {
						...
						var r = function(i, t) {
							if (1 == i) {
								...
								var a = setInterval(function() {
									r--;
									$(e).text(r + '秒后重发'),
									0 == r && ($(e).removeClass('disabled'), $(e).text('免费获取安全码'),$(e).siblings('span').removeClass().text(''), clearInterval(a))
								},1e3)
							} 
						};						
					}
			...
			$('#registerByPhoneSubmitPop').click(function () {
				var a=$('#registerPhonePop').val();
				var b=$('#registerSafeCodePop').val();
				var c=$('#registerPhonePassPop').val();
				var d=$('#registerNameVcodePop').val();
				...
				$.ajax({
					...
					data: {'account':a,'vcode':b,'password':c,'r_quan':d,},
					beforeSend: function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    case 1:
                        ...
                        $("#copy_code").zclip({
                            path: "/Public/Media2/js/ZeroClipboard.swf",
                            copy: function() {
