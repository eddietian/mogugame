...
function get_game_coin(){
	var money = parseInt($('#money1').val());

	if(!money){
		money = parseInt($('#jubaobar input[name="money"]:checked').val());
	}else{
		$('#jubaobar input[name="money"]:checked').attr('checked', false).parent().removeClass('money_checked');
	}
	
	if(isNaN(money)) money = 0;
	$('#game_coin').html(money * 1);
}
...
$(function(){
	...
	//聚宝云
	$("#jubaobar_action").on("click", function(){
		
		if($.trim($("#toaccount1").val()).length == 0){
			alert("充值账号不能为空!");
			return false;
		}

		if($.trim($("#retoaccount1").val()).length != $.trim($("#toaccount1").val()).length){
			alert("两次账号不相同!");
			return false;
		}

		if($('#jubaobar input[name="money"]:checked').val() == undefined){
			if($("#money1").val() == 0){
				alert("请输入充值金额");
				return false;
			}
		}
		var $amount = $('#jubaobar input[name="money"]:checked').val() == undefined?$("#money1").val():$('#jubaobar input[name="money"]:checked').val()
		$("#jubaobar_amount").val($amount);
		$("#form_jubaobar").submit();
	...
	//竣付通
	$("#jft_action").on("click", function(){
		
		if($.trim($("#toaccount11").val()).length == 0){
			alert("充值账号不能为空!");
			return false;
		}

		if($.trim($("#retoaccount11").val()).length != $.trim($("#toaccount11").val()).length){
			alert("两次账号不相同!");
			return false;
		}

		if($('#jft input[name="money"]:checked').val() == undefined){
			if($("#money1").val() == 0){
				alert("请输入充值金额");
				return false;
			}
		}
		var amount = $('#jft input[name="money"]:checked').val() == undefined?$("#money11").val():$('#jft input[name="money"]:checked').val();

		$("#jft_amount").val(amount);
		$("#form_jft").submit();
