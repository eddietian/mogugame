      ...
      if (get_tool_status("oss_storage") == 1) {
        if(!empty($picture['oss_url'])){
           return $picture['oss_url'];
            ...
            return $http_type.'//' . $_SERVER['HTTP_HOST'] . __ROOT__.$picture['path'];
        }
...
function age($cardno,$name){
    /*$arr = array(
        'data'=>array('sex'=>'F','address'=>'江苏省徐州市沛县','birthday'=>'1999-08-18'),
        'resp'=>array('code'=>0,'desc'=>'匹配')
    );
    return $arr;*/
...
function sdkchecksendcode($phone,$limit){
    $number = M('Short_message')->where(array('pid'=>0,'create_ip'=>get_client_ip(),'send_status'=>'000000','send_time'=>array(array('egt',strtotime('today')),array('elt',strtotime('tomorrow')))))->count();
   if (!empty($limit) && $number>=$limit) {
        $return['status'] = '0';
        $return['msg']='每天发送数量不能超过'.$limit.'条';
    ...
    $number = $time = M('Short_message')->where($map)->count();
    if ($number>0){
        echo base64_encode(json_encode(array('status'=>0,'msg'=>'请一分钟后再次尝试')));exit;
    }
}
function sdk2checksendcode($phone,$limit){
   ...
   if (!empty($limit) && $number>=$limit) {
    ...
    $number = $time = M('Short_message')->where($map)->count();
    if ($number>0){
        echo base64_encode(json_encode(array('status'=>0,'return_msg'=>'请一分钟后再次尝试')));exit;
    }
}
function appchecksendcode($phone,$limit){
    $number = M('Short_message')->where(array('pid'=>0,'create_ip'=>get_client_ip(),'send_status'=>'000000','send_time'=>array(array('egt',strtotime('today')),array('elt',strtotime('tomorrow')))))->count();
    if (!empty($limit) && $number>=$limit) {
        $msg = array(
            "status" => -1,
            "return_code" => "每天发送数量不能超过".$limit."条",
            "data" => []
        );
